...
// Node represents DoublyLinkedList node
type Node struct {
	next  *Node
	prev  *Node
	value float64
...
func (d *DoublyLinkedList) InsertAfter(item float64, node *Node) error {
	if d.head == nil {
...
func (d *DoublyLinkedList) Delete(node *Node) error {
	if d.head == nil {
		return errors.New("List is empty")
	}
	if d.head == node {
...
func (d *DoublyLinkedList) Search(item float64) (*Node, error) {
	if d.head == nil {
...
// Update node value
func (d *DoublyLinkedList) Update(item float64, node *Node) {
	node.value = item
}
...
// Test is DoublyLinkedList implememntation test
func Test(l DoublyLinkedList) {
